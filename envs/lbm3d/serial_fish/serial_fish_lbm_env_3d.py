...
# ============== Warp Kernels for Serial Fish Environment ==============


@wp.kernel
def compute_serial_fish_obs_kernel(
    ...
    """
    Compute observation for serial fish.
    
    Observation layout (n_joints=5, with pitch control):
    ...
    Total: 6 + 5 + 3 + 4 + 3 + 3 + 5 + 5 + 3 + 3 = 40 dims (for 5 joints)
...
@wp.kernel
def check_boundary_serial_fish_kernel(
    flows: wp.array(dtype=HomeFlow3D),
...
@wp.kernel
def check_goal_reached_serial_fish_kernel(
    flows: wp.array(dtype=HomeFlow3D),
...
@wp.kernel
def check_stability_serial_fish_kernel(
...
@wp.kernel
def compute_goal_reward_serial_fish_kernel(
    flows: wp.array(dtype=HomeFlow3D),
    goal_positions: wp.array2d(dtype=wp.float32),
...
@wp.kernel
def compute_smooth_reward_serial_fish_kernel(
    qvel: wp.array2d(dtype=wp.float32),  # (nworld, nv)
    qpos: wp.array2d(dtype=wp.float32),  # (nworld, nq)
    ...
    pitch_alignment_weight: float,
    action_dim: int,
    nz: float,
):
    """
    Compute smoothness reward for serial fish with traveling wave and pitch control.
    ...
    3. Traveling wave reward: encourage phase difference between yaw joints
    ...
    - qvel[6]: seg1_pitch velocity
    - qvel[7]: seg1_yaw velocity
    - qvel[8]: seg2_yaw velocity
    - qvel[9]: seg3_yaw velocity
    - qvel[10]: tail_yaw velocity
    
    qpos layout:
    - qpos[0:3]: position (x, y, z)
    - qpos[3:7]: quaternion (w, x, y, z)
    - qpos[7]: seg1_pitch angle
    - qpos[8]: seg1_yaw angle
    - qpos[9]: seg2_yaw angle
    - qpos[10]: seg3_yaw angle
    - qpos[11]: tail_yaw angle
    """
    world_idx = wp.tid()
    ...
    # ========== 2. Rotation Penalty (mainly penalize roll) ==========
    # Allow yaw rotation for turning, penalize roll more
    omega_x = qvel[world_idx, 3]  # roll angular velocity
    omega_y = qvel[world_idx, 4]  # pitch angular velocity (allow some)
    omega_z = qvel[world_idx, 5]  # yaw angular velocity (allow for turning)
    # Penalize roll heavily, yaw lightly
    rotation_penalty = -rotation_penalty_weight * (4.0 * omega_x * omega_x + omega_y * omega_y + 0.25 * omega_z * omega_z)
    
    # ========== 3. Traveling Wave Reward ==========
    # Encourage phase difference between consecutive yaw joints
    # For efficient swimming: wave propagates from head to tail
    # Reward when adjacent joints have opposite phase (one moving while other reversing)
    #
    # Yaw joint velocities: qvel[7], qvel[8], qvel[9], qvel[10]
    # Yaw actions: action[1], action[2], action[3], action[4]
    
    wave_reward = float(0.0)
    
    # Phase difference between seg1_yaw and seg2_yaw
    phase_diff_12 = qvel[world_idx, 7] * current_actions[world_idx, 2] - qvel[world_idx, 8] * current_actions[world_idx, 1]
    wave_reward = wave_reward + wp.abs(phase_diff_12)
    
    # Phase difference between seg2_yaw and seg3_yaw
    phase_diff_23 = qvel[world_idx, 8] * current_actions[world_idx, 3] - qvel[world_idx, 9] * current_actions[world_idx, 2]
    wave_reward = wave_reward + wp.abs(phase_diff_23)
    
    # Phase difference between seg3_yaw and tail_yaw
    phase_diff_34 = qvel[world_idx, 9] * current_actions[world_idx, 4] - qvel[world_idx, 10] * current_actions[world_idx, 3]
    wave_reward = wave_reward + wp.abs(phase_diff_34)
    
    wave_reward = wave_reward_weight * wave_reward
    
    # ========== 4. Pitch Alignment Reward ==========
    # Encourage pitch angle to align with vertical goal direction
    # When goal is above: positive pitch (head up) → positive reward
    # When goal is below: negative pitch (head down) → positive reward
    #
    # Get current z position and goal z
    flow = flows[world_idx]
    head_pos = flow.solid_position[0]
    current_z = head_pos[2]
    goal_z = goal_positions[world_idx, 2] * nz
    
    # Vertical direction: positive if goal is above, negative if below
    z_diff = goal_z - current_z
    ...
    # Pitch angle: qpos[7], positive pitch = head up
    pitch_angle = qpos[world_idx, 7]
    
    # Reward when pitch direction matches z direction
    # pitch > 0 and goal above (z_direction > 0) → good
    # pitch < 0 and goal below (z_direction < 0) → good
    pitch_alignment = pitch_alignment_weight * pitch_angle * z_direction
    
    # ========== Total Smooth Reward ==========
    smooth_rewards_out[world_idx] = action_smooth_reward + rotation_penalty + wave_reward + pitch_alignment
...
class SerialFish3DLBMEnv(LBMFluidEnv3D):
    """
    ...
    - Body segment 1 (box): with pitch + yaw joints (for 3D maneuvering)
    - Body segments 2-3 (boxes): each with yaw joint
    - Tail peduncle (capsule): with yaw joint
    - Tail fin (ellipsoid): attached to peduncle
    
    Control: 5 joints total
    - 1 pitch joint (seg1_pitch): controls up/down motion
    - 4 yaw joints (seg1_yaw ~ tail_yaw): wave-like swimming motion
    
    Goals:
    - Multiple goal points in a 3x3x3 grid (excluding center where robot starts)
    - Robot starts from center
    - When reaching a goal, next goal is selected (multi-goal mode)
    - Or terminate on goal reach (single goal mode)
    """
    ...
    def __init__(
        ...
        """
        Initialize Serial Fish 3D LBM Environment.
        ...
        Args:
            ...
            single_goal_mode: If True, terminate when goal reached; if False, continue to next goal
        ...
        # Use serial_fish_3d.xml from the same folder
        if mjcf_path is None:
            mjcf_path = os.path.join(os.path.dirname(__file__), 'serial_fish_3d.xml')
        ...
        # Number of joints (5 joints for serial fish: 1 pitch + 4 yaw)
        ...
        # Observation space: 6 + n_joints + 3 + 4 + 3 + 3 + n_joints + n_joints + 3 + 3
        # = 25 + 3 * n_joints = 25 + 15 = 40 dims for 5 joints
        ...
        if self.single_goal_mode:
            # Single goal: use provided goal_position
            gp = self._init_goal_position
            self.goal_positions_list = [(gp[0], gp[1], gp[2])]
        else:
            # Multi-goal: 3x3x3 grid (normalized coordinates), excluding center
        ...
        self.wave_reward_weight = 0.0001      # Traveling wave reward weight
        self.pitch_alignment_weight = 0.5     # Pitch alignment with vertical goal direction
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        wp.launch(
            compute_serial_fish_obs_kernel,
            dim=self.nworld,
            inputs=[
    ...
    def _check_goals_reached(self):
        ...
        wp.launch(
            check_goal_reached_serial_fish_kernel,
            dim=self.nworld,
            inputs=[
                self.lbm_solver.flows_wp,
                self._goal_positions_wp,
    ...
    def _compute_reward(self, instability_mask: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        wp.launch(
            compute_goal_reward_serial_fish_kernel,
            dim=self.nworld,
            inputs=[
                self.lbm_solver.flows_wp,
                self._goal_positions_wp,
        ...
        # Smoothness reward (includes traveling wave and pitch alignment)
        if self._prev_actions_wp is not None and self._current_actions_wp is not None:
            wp.launch(
                compute_smooth_reward_serial_fish_kernel,
                dim=self.nworld,
                inputs=[
                    self.mjw_data.qvel,
                    self.mjw_data.qpos,
    ...
    def _is_terminated(self, instability_mask: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        wp.launch(
            check_boundary_serial_fish_kernel,
            dim=self.nworld,
            inputs=[
                self.lbm_solver.flows_wp,
                self._terminated_buffer,
    ...
    def _check_numerical_stability(self) -> np.ndarray:
        ...
        wp.launch(
            check_stability_serial_fish_kernel,
            dim=self.nworld,
            inputs=[
                self.mjw_data.qpos,
                self.mjw_data.qvel,
    ...
    def step(self, action: np.ndarray):
        ...
        if self.single_goal_mode and np.any(goal_reached):
            goal_reached_mask = goal_reached.astype(bool)
            reward[goal_reached_mask] += self.goal_reached_bonus  # Extra bonus
            terminated = terminated | goal_reached_mask
        
        # Termination penalty (only for non-goal terminations)
        non_goal_terminated = terminated & ~goal_reached.astype(bool) if self.single_goal_mode else terminated
