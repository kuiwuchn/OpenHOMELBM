"""
3D Clownfish LBM Environment for MuJoCo Warp with nworld support.

Clownfish (Nemo) swimming with 4-segment articulated body.
Based on the Turtle/Tuna implementation pattern (Multi-Goal reward).
...
# ============== Warp Kernels for 3D Clownfish Environment ==============


@wp.kernel
def compute_clownfish_obs_3d_kernel(
    ...
    Observation layout for 3D Clownfish:
...
@wp.kernel
def check_boundary_3d_clownfish_kernel(
    flows: wp.array(dtype=HomeFlow3D),
...
@wp.kernel
def check_stability_3d_clownfish_kernel(
    qpos: wp.array2d(dtype=wp.float32),
    qvel: wp.array2d(dtype=wp.float32),
...
@wp.kernel
def check_goal_reached_clownfish_kernel(
    flows: wp.array(dtype=HomeFlow3D),
    goal_positions: wp.array2d(dtype=wp.float32),
    ...
    """Check if clownfish reached its goal."""
    world_idx = wp.tid()
    flow = flows[world_idx]
...
@wp.kernel
def apply_instability_penalty_clownfish_kernel(
...
@wp.kernel
def compute_goal_reward_clownfish_kernel(
    flows: wp.array(dtype=HomeFlow3D),
    goal_positions: wp.array2d(dtype=wp.float32),
    ...
    for i in range(n_joints):
        action_penalty += 0.01 * qfrc_applied[world_idx, 6 + i] * qfrc_applied[world_idx, 6 + i] # L2 penalty on joint forces
    
    head_w = qpos[world_idx, 3]
    head_x = qpos[world_idx, 4]
    head_y = qpos[world_idx, 5]
    head_z = qpos[world_idx, 6]
    
    
    head_forward_vec = wp.vec3(2.0 * (head_x * head_y - head_w * head_z), 
                                1.0 - 2.0 * (head_x * head_x + head_z * head_z), 
                                2.0 * (head_y * head_z + head_w * head_x))
...
@wp.kernel
def reset_prev_dist_clownfish_kernel(
    flows: wp.array(dtype=HomeFlow3D),
    goal_positions: wp.array2d(dtype=wp.float32),
...
@wp.kernel
def init_prev_dist_clownfish_kernel(
    flows: wp.array(dtype=HomeFlow3D),
    goal_positions: wp.array2d(dtype=wp.float32),
    prev_dist: wp.array(dtype=wp.float32),
    nx: float,
...
# ============== 3D Clownfish LBM Environment Class ==============


class Clownfish3DLBMEnv(LBMFluidEnv3D):
    """
    3D Clownfish swimming environment with LBM fluid simulation.
    Multi-Goal reward: distance improvement toward goal + goal reached bonus.

    Structure:
    - Head (root body with freejoint)
    - Body (2DOF: yaw + pitch)
    - Tail Peduncle (2DOF: yaw + pitch)
    - Caudal Fin (2DOF: yaw + pitch)

    Actuators (6 DOF):
      Body:      body_yaw, body_pitch       (2)
      Peduncle:  peduncle_yaw, peduncle_pitch (2)
      Caudal:    caudal_yaw, caudal_pitch    (2)
    ...
    def __init__(
        ...
        if mjcf_path is None:
            mjcf_path = os.path.join(os.path.dirname(__file__), 'clownfish_3d.xml')
        ...
        print(f"Clownfish3DLBMEnv initialized:")
    ...
    def _compute_reward(self, instability_mask=None) -> np.ndarray:
        ...
        wp.launch(
            compute_goal_reward_clownfish_kernel,
            dim=self.nworld,
            inputs=[
                self.lbm_solver.flows_wp,
                self._goal_positions_wp,
    ...
    def _check_goals_reached(self) -> np.ndarray:
        ...
        wp.launch(
            check_goal_reached_clownfish_kernel,
            dim=self.nworld,
            inputs=[
                self.lbm_solver.flows_wp,
                self._goal_positions_wp,
        ...
        if not self.single_goal_mode and np.any(goal_reached):
            ...
            wp.launch(
                reset_prev_dist_clownfish_kernel,
                dim=self.nworld,
                inputs=[
                    self.lbm_solver.flows_wp,
                    self._goal_positions_wp,
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None) -> np.ndarray:
        ...
        wp.launch(
            init_prev_dist_clownfish_kernel,
            dim=self.nworld,
            inputs=[
                self.lbm_solver.flows_wp,
                self._goal_positions_wp,
